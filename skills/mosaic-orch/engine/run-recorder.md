...
mosaic.json を表示（`--mosaic`）する際、旧エントリに `stages_run`, `total_stages`, `duration_s` が存在しない場合がある。
- 欠落フィールドはスキップする（エラーにしない）
- `duration_s` 欠落エントリは平均計算から除外する
- 未知のフィールド（旧エントリの `task` 等）は無視する
