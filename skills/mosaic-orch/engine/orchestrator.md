...
- `dryRun`: boolean
- `resumeRunDir`: string | null（`--resume` 時のみ、既存ランディレクトリパス）

## 状態機械

```
[INIT] → [RESOLVE_STAGE] → [CHECK_WHEN] → [CHECK_GATE] → [RUN_STAGE] → [CHECK_LOOP] → [ADVANCE] → [COMPLETE]
                                   ↓ false       ↓ denied                                    ↑
                                 [SKIP] ─────────┴──────────────────────────────────────────┘
エラー発生時 → [ABORT]
```
...
2. yaml-schema.md の静的検証ルール（V1〜V17）を全て実行する
...
4. **resume モード判定**:
   ...
   **resumeRunDir が指定されている場合（--resume）:**
   a. run-recorder.loadCheckpoint(resumeRunDir) を呼ぶ → { stageIndex, stagesRunCount, stageHistory, context } を取得
   b. context に `workflow.inputs` をマージする（inputs は常に最新を使う）
   c. runDir = resumeRunDir
   d. ユーザーに報告: `🔄 Resuming from stage {stages[stageIndex].id} (run: {runDir})`
   e. 手順 7 へスキップ

   **resumeRunDir が null の場合（通常モード）:**
   a. 変数コンテキストを初期化する:
      ```
      context = {
      ...
      ```
   b. run-recorder.initRun() を呼ぶ → runDir を取得
   c. stageIndex = 0 に設定
   d. stageHistory = []
   e. stagesRunCount = 0

7. stages 配列を取得
8. loopMonitors = workflow.loop_monitors || []
9. startTime = 現在時刻（COMPLETE/ABORT 時の duration 計算用）
...
1. currentStage.when が未定義 → [CHECK_GATE] に進む
2. when 条件を評価する（stage-runner.md の比較演算ルールを使う）
3. true → [CHECK_GATE] に進む
4. false → [SKIP] に進む

### CHECK_GATE

1. currentStage.gate が未定義 → [RUN_STAGE] に進む
2. gate == "approval" の場合:
   a. ユーザーに AskUserQuestion で確認を求める:
      ```
      ⏸️ 承認ゲート: {currentStage.id}
      
      次のステージを実行します:
      - ステージ: {currentStage.id} ({currentStage.kind})
      - 入力: {展開前の input の先頭200文字}
      ...
      ```
   b. ユーザーの回答に応じて:
      - 「承認」「続行」「1」 → run-recorder.recordEvent(runDir, { event: "gate_approved", stage_id }) → [RUN_STAGE]
      - 「スキップ」「2」 → run-recorder.recordEvent(runDir, { event: "gate_skipped", stage_id }) → [SKIP]
      - 「中断」「3」 → [ABORT]
...
0. **残りの変数展開**: currentStage の input, from, loop_until, skills, mcps を variable-resolver で展開する
   - mcps が `${...}` 形式の変数参照で、値が undefined/null の場合: mcps を空配列にする（エラーにしない。MCP未割り当てのサブタスクがあるのは正常）
...
5. run-recorder.writeCheckpoint(runDir, { stageIndex: stageIndex + 1, stagesRunCount, stageHistory, context })
