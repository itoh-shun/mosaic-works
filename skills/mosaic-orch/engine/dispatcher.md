...
Claude Code の Task tool は **1メッセージで同時起動できる数に実用上の上限がある**。
以下のガイドラインに従うこと:

| prompts 数 | 推奨設定 | 備考 |
|---|---|---|
| 1〜5 | 全部並列（maxParallel 不要） | 通常のユースケース |
| 6〜10 | maxParallel: 5 を推奨 | タイムアウト・コンテキスト枯渇のリスク上昇 |
| 11 以上 | maxParallel: 5 以下を強く推奨 | バッチ実行で安定性を確保する |

> **注意**: maxParallel が null（無制限）のまま 10 を超える要素を fan_out すると、
> Task tool のタイムアウトやオーケストレーターのコンテキスト上限に達する可能性がある。
> Workflow YAML 作成者は `max_parallel: 5` を明示することを推奨する。
